...
|---------|-------------|
| `/blueprint` | Create implementation plan with architecture design, task breakdown, risk assessment — **waits for confirm before touching code** |
...
| `/consult` | Debug partner and technical advisor — read-only analysis, root-cause investigation, provide insight on architecture, system design |
...
```
Starting a feature?              → /blueprint first, then /implement --mode=full
...
What architecture should be used here? → /consult
Critical system design decision → /consult
