...
## Coding-convention
In one ts component, always apply the sorting:
- Template decorators ViewChild, ContentChild,...
- Properties decorators @Input
- Event emitter decorators @Output
- Variables (inner states of component)
  - injections(private readonly)
  - private readonly variable (constants)
  - private variable (mutable)
  - public readonly (constants)
  - public variable (mutable)
- Constructor(If exists)
- Lifecycle hook: ngOnChanges, ngOnInit, ngAfterViewInit, ngOnDestroy,...
- HostListeners(If exists)
- public methods
- private methods(JS doc explains why, not what)
- private getter
- public getter

Modifier (private/public/protected) need to be fully defined explicitly.

When passing input to children html template, always use input binding(`[id]=''component-id''`) to avoid misunderstanding for the default properties of the element.

Always prefer classes in template over SCSS when styling the template to avoid messy style attributes. The classes normally comes from sources:
- global style: styles.scss
- framework style: [primeflex](https://primeflex.org/)
- when missing some custom style, apply it at the component level if it is not generalized, or else, place it in the global style to use.
Avoid apply changes ito .scss, only when the ::ng-deep is needed. ::ng-deep have to always go with :host to avoid correlation change causing unexpected behavior.
Avoid using padding, margin. They should only be applied at outline layer.
Avoid using optional chaining on template.
