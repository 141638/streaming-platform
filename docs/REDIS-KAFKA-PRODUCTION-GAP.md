...
| # | Gap | Severity | Tier | Linked ADR | What to do |
|---|-----|----------|------|------------|------------|
| K1 | **At-most-once delivery** — if Kafka is unreachable for 2s, the event is silently dropped. PG write succeeded, HTTP response is 200, but `notification-service` never learns the stream started. `.onErrorComplete()` swallows the failure. | **CRITICAL** | T2 | [ADR-stream-0002 § Trade-off](adr/stream/0002-kafka-event-publishing.md) | **#1 priority.** Implement the Outbox Pattern: (a) `outbox` table in `stream` schema, (b) write in same transaction as entity change, (c) scheduled poller with `SELECT ... FOR UPDATE SKIP LOCKED`, (d) publish to Kafka + delete from outbox on success + retry with backoff on failure. OR: evaluate Debezium CDC for zero-code change capture. |
| K2 | **No Dead Letter Queue** — poison pill messages (malformed JSON, missing stream, processing failure) cause infinite retry, blocking the partition. | **HIGH** | T2 | — | Add `SeekToCurrentErrorHandler` with `DeadLetterPublishingRecoverer`. After N retries (default: 10), publish to `stream.control.dlq` and move on. Add DLQ monitoring — any message in DLQ = alert. |
| K3 | **No idempotent producer** — network blip between `send()` and broker `ack` causes retry, which may produce a duplicate. No `enable.idempotence=true`, no `acks=all`. | HIGH | T2 | [ADR-stream-0002 § Deferred Concerns §2](adr/stream/0002-kafka-event-publishing.md) | Add `enable.idempotence=true` + `acks=all` to producer config. The `eventId` on the envelope already enables consumer-side dedup; this adds producer-side dedup as defense-in-depth. |
...
| K10 | **Fire-and-forget lifecycle issue** — `.doOnSuccess(saved -> eventPublisher.publish(event).subscribe())` creates a "hot" subscription. If the HTTP response completes and Reactor context is cleaned before Kafka `send()` finishes, the Mono may cancel mid-flight. | MEDIUM | T2 | [ADR-stream-0002 § Consequences](adr/stream/0002-kafka-event-publishing.md) | Subscribe on `Schedulers.boundedElastic()` explicitly, or wire the publish as a `Mono.fromRunnable()` side effect that survives context cancellation. Test: add `Thread.sleep(5000)` in the Kafka send path and verify the event still publishes after the HTTP response returns. |
