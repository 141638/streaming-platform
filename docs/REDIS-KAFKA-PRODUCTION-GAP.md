...
| R8 | **Ephemeral data store use cases partially implemented** — ~~refresh tokens~~ ✅ (6.0b), ~~rate limiting~~ ✅ (6.3), ~~idempotency keys~~ ✅ (notification-service `ModerationEventListener` Redis SETNX dedup, 24h TTL, 2026-08-08), ~~event coalescing~~ ✅ (notification-service `ModerationEventCoalescer` SETNX 5s window, 2026-08-08). WebSocket session affinity and distributed locking are still not built. | LOW | T2 | [ADR-common-0002](adr/common/0002-redis-ephemeral-data-store.md) | Remaining: WebSocket session affinity (Redis Pub/Sub already handles fan-out), distributed locking (no current use case).
...
| K7 | ~~**`AUTO_CREATE_TOPICS_ENABLE: true`**~~ ✅ **DONE (2026-08-01, C3).** Disabled in all 5 broker configs + `spring.kafka.admin.auto-create: false` in stream-service. Topics provisioned declaratively via `@Bean NewTopic` in `common/.../messaging/KafkaTopicConfig.java` (shared-library approach — no broker restart needed for new topics). | ~~MEDIUM~~ ✅ | T2 | — | Done. See [C3 retrospective](plans/C3-kafka-topic-provisioning-retrospective.md) and [Kafka infrastructure doc](architecture/kafka-infrastructure.md). |
...
| 7 | **K7 — Disable auto-create topics** ✅ | ~30 min | Done (2026-08-01). Shared-library approach via `common/.../KafkaTopicConfig.java`. |
