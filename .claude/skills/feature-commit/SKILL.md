...
## Telemetry Isolation (PRE-STEP)

`docs/.telemetry.json` is a side-effect artifact generated by the Stop hook. It tracks agent/skill usage counts — it is **never** part of any feature's source tree.

**Rule:** Do NOT bundle `docs/.telemetry.json` into any feature commit. When it appears in the working tree:
- Commit it **separately** as `chore: update telemetry tracker` — before or after the feature batch
- Or leave it for the user to handle manually
...
| **Bundling `docs/.telemetry.json` with feature files** | Pollutes feature history with hook side-effect churn | Commit it in isolation (`chore: update telemetry tracker`) or leave it |
